...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    #Here we find the ht for current input xt using previous state
...
def rnn_step_backward(dnext_h, cache):
    ...
    #Compute gradient of tanh wrt next_h
    ...
    dxWx = dtanh
    dphWh = dtanh
    #Compute gradient wrt wh
    dWh = prev_h.T.dot(dphWh)
    dprev_h = Wh.dot(dphWh.T).T
    #Compute gradient wrt Wx
    dx = dxWx.dot(Wx.T)
    dWx = x.T.dot(dxWx) 
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
        ...
        next_h, chache_step = rnn_step_forward(x[:,t,:], next_h, Wx, Wh, b)
        #Save the value of the new next hidden state
        h[:,t,:] = next_h
        cache.append(chache_step)
...
def rnn_backward(dh, cache):
    ...
    # Initialization the gradiets with apt shape
