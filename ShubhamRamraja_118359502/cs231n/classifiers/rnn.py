...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        #Use  vanilla RNN forward
        rnn_forward_out, rnn_forward_cache = rnn_forward(word_embed_out, affine_out, Wx, Wh, b)
        #Use temporal affine to find score
        temporal_affine_out, temporal_affine_cache = temporal_affine_forward(rnn_forward_out, W_vocab, b_vocab)
        #Use temporal softmax
        loss, dout = temporal_softmax_loss(temporal_affine_out, captions_out, mask, False)
        #Calculate gradients
        d_rnn, dW_vocab, db_vocab = temporal_affine_backward(dout, temporal_affine_cache)
        #RNN backward pass
        dx, dh0, dWx, dWh, db = rnn_backward(d_rnn, rnn_forward_cache)
        #Embed backward
        dW_embed = word_embedding_backward(dx, word_embed_cache)
        dout, dW_proj, db_proj = affine_backward(dh0, affine_cache)
        #Store the gradients
    ...
    def sample(self, features, max_length=30):
        ...
        for i in range(max_length):
            ...
            #Make an RNN step using the previous hidden state 
            h0, _ = rnn_step_forward(x, h0, Wx, Wh, b)
           
            # Apply affine transform and get scores 
